...
-- Ordene seus resultados por nomes de forma decrescente. Exiba somente os
-- clientes ativos. As informações podem ser encontradas na tabela address e customer .
SELECT 
    C.first_name, COUNT(A.address)
FROM
    sakila.address AS A
        INNER JOIN
    sakila.customer AS C ON C.address_id = A.address_id
WHERE
    C.active = 1
...
SELECT 
    ACT.actor_id, ACT.first_name, FAC.film_id, FIL.title
FROM
    sakila.actor AS ACT
        INNER JOIN
    sakila.film_actor AS FAC ON ACT.actor_id = FAC.actor_id
        INNER JOIN
    sakila.film AS FIL ON FAC.film_id = FIL.film_id;
